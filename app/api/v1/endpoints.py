...
def get_access_token(creds: HTTPAuthorizationCredentials = Depends(security)) -> str:
    return creds.credentials


@inject
async def current_user(service: FromDishka[ShowCurrentUserService], token: str = Depends(get_access_token)) -> 'UserModel':
    return await service.show_current_user(token)


@inject
async def current_admin(service: FromDishka[ShowCurrentUserService], token: str = Depends(get_access_token)) -> 'UserModel':

...
class WalletQueriesRepository:
    ...
    async def select_wallets_by_user_id(self, user_id: UUID, offset: int = 0, limit: int = 100) -> list['WalletModel']:
        objs  = await self._async_session.execute(select(WalletModel).where(WalletModel.user_id == user_id).offset(offset).limit(limit))
